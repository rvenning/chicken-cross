...
// with zero dependencies: shapes are rasterized into an RGBA buffer at 4x
// resolution, box-downsampled for antialiasing, and written as PNG using
// Node's built-in zlib for the IDAT stream.
//
...
function makeCanvas(size) {
  ...
  const set = (x, y, [r, g, b], a = 1) => {
    ...
    const i = (y * size + x) * 4;
    // source-over blend so soft shadows layer correctly
...
// 4x box downsample: render big, average down — cheap antialiasing.
function downsample(src, srcSize, factor) {
  const out = Buffer.alloc((srcSize / factor) ** 2 * 4);
